...
yarn test
```
To edit the Objective-C files, open `example/ios/SdkxExample.xcworkspace` in XCode and find the source files at `Pods > Development Pods > react-native-sdkx`.

To edit the Kotlin files, open `example/android` in Android studio and find the source files at `reactnativesdkx` under `Android`.
